...
# --- Integrated Chat and Admin Interface ---

# Define available admin commands.
ADMIN_COMMANDS = {
    "/rebuild_docs": rebuild_docs,
    "/get_golden_qa_pairs": get_golden_qa_pairs,
    "/update_golden_qa_pairs": update_golden_qa_pairs,
    "/get_conversations": get_conversations
}

def execute_admin_command(command, encrypted_key, product, endorsed_by="user"):
    if command == "/rebuild_docs":
        return rebuild_docs(encrypted_key)
    elif command == "/get_golden_qa_pairs":
        return get_golden_qa_pairs(product, encrypted_key)
    elif command == "/update_golden_qa_pairs":
        return update_golden_qa_pairs(product, encrypted_key)
    elif command == "/get_conversations":
        content, _, _ = get_conversations(product, endorsed_by, encrypted_key)
        return content
    else:
        return "Unknown admin command."

def chat(nl_query, history, session_id, locale, product, is_expert, encrypted_key_input):
    nl_query = nl_query.strip()
    if not nl_query:
        return history, session_id, gr.update(visible=False)

    # Handle admin commands
    if nl_query.startswith("/"):
        parts = nl_query.split(maxsplit=1)
        command = parts[0]
        if command in ADMIN_COMMANDS:
            if len(parts) > 1:
                encrypted_key = parts[1].strip()
                result = execute_admin_command(command, encrypted_key, product)
                history.append((nl_query, result))
                return history, session_id, gr.update(visible=False)
            else:
                # Show encrypted key input if no key is provided
                history.append((nl_query, f"Valid admin command '{command}' detected. Please provide your encrypted key below."))
                return history, session_id, gr.update(visible=True, value="", placeholder=f"Enter encrypted key for {command}")
        else:
            history.append((nl_query, "Unrecognized admin command."))
            return history, session_id, gr.update(visible=False)

    # Normal chat query processing
...
def handle_key_submission(nl_query, encrypted_key, history, session_id, locale, product, is_expert):
    if not encrypted_key:
        history.append((None, "Please enter a valid encrypted key."))
        return history, session_id, gr.update(visible=True)
    parts = nl_query.split(maxsplit=1)
    command = parts[0]
    result = execute_admin_command(command, encrypted_key, product)
    history.append((f"{command} {encrypted_key}", result))
    return history, session_id, gr.update(visible=False)
...
def update_dropdown(text):
    if text.startswith("/"):
        typed_command = text[1:].lower()
        matching_commands = [cmd for cmd in ADMIN_COMMANDS.keys() if cmd.lower().startswith(typed_command)]
        return gr.update(visible=True, choices=matching_commands, interactive=True)
    else:
        return gr.update(visible=False, interactive=False)

def select_command(selected):
    return selected
...
with gr.Blocks(css=custom_css, title="Lil Lisa Server") as demo:
    ...
    with gr.Tab("Chatbot"):
        ...
        # Place the natural query input and the admin command dropdown side by side
        with gr.Row():
            nl_query_input = gr.Textbox(placeholder="Type your query here...", label="Query")
            admin_command_dropdown = gr.Dropdown(choices=[], label="", visible=False)
        
        is_expert_answering = gr.Checkbox(label="Is Expert Answering?", value=False)
        encrypted_key_input = gr.Textbox(placeholder="Enter encrypted key...", label="Encrypted Key", visible=False)
        
        with gr.Row():
            send_btn = gr.Button("Send Query")
            submit_key_btn = gr.Button("Submit Key", visible=False)
            clear_btn = gr.Button("Clear Chat")
            endorse_btn = gr.Button("Endorse Conversation")
        endorse_type = gr.Radio(["User", "Expert"], label="Endorsement Type", value="User")

        # When the natural query input changes, update the inline dropdown if it starts with "/"
        nl_query_input.change(update_dropdown, inputs=nl_query_input, outputs=admin_command_dropdown)
        
        # When a command is selected from the dropdown, update the natural query input with that command
        admin_command_dropdown.change(select_command, inputs=admin_command_dropdown, outputs=nl_query_input)
        
        # The main chat button will process the input; if an admin command is detected but no key provided,
        # it will show the encrypted key input.
        send_btn.click(
            chat,
            inputs=[nl_query_input, chatbot, session_id_state, locale_dropdown, product_dropdown, is_expert_answering, encrypted_key_input],
            outputs=[chatbot, session_id_state, encrypted_key_input]
        ).then(
            lambda key: gr.update(visible=bool(key)),
            inputs=[encrypted_key_input],
            outputs=[submit_key_btn]
        )
        
        # Handle submission of the encrypted key
        submit_key_btn.click(
            handle_key_submission,
            inputs=[nl_query_input, encrypted_key_input, chatbot, session_id_state, locale_dropdown, product_dropdown, is_expert_answering],
            outputs=[chatbot, session_id_state, encrypted_key_input]
        ).then(
            lambda key: gr.update(visible=bool(key)),
            inputs=[encrypted_key_input],
        ...
        clear_btn.click(
            ...
            None,
            [chatbot, session_id_state, encrypted_key_input, submit_key_btn]
        )
        
        # Endorse conversation
