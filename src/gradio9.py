...
# --- New helper function for key verification ---
...
def invoke(session_id: str, locale: str, product: str, nl_query: str, is_expert_answering: bool) -> str:
    try:
        ...
        # If locale is "fr-FR", add an extra instruction to respond in French
        if locale == "fr-FR":
            react_agent_prompt += (
        ...
        # Now proceed with your ReActAgent call
...
def get_golden_qa_pairs(product: str, encrypted_key: str):
    try:
        ...
        if os.path.isfile(filepath) and os.path.getsize(filepath) > 0:
            ...
            # Return the ZIP content to Gradio
            return zip_stream.getvalue(), "application/zip", "qa_pairs.zip"

        # If file not found or empty
        return (
            "No QA pairs found for this product. Please check the repository or try another product.",
            "text/plain",
            "error.txt"
        )
...
# --- New Helper Functions for Thumbs Up/Down Feedback ---
def handle_thumbs_up(session_id: str) -> str:
    ...
    try:
        ...
        llsc = get_llsc(session_id)
        # Concatenate conversation history into a single Markdown-formatted string
        conversation_history = "\n".join(f"{poster}: {message}" for poster, message in llsc.conversation_history)
        # Determine the target folder (placed in the same parent directory as SPEEDICT_FOLDERPATH)
        base_folder = os.path.dirname(LilLisaServerContext.SPEEDICT_FOLDERPATH)
        thumbsup_folder = os.path.join(base_folder, "Thumbsup")
        if not os.path.exists(thumbsup_folder):
            os.makedirs(thumbsup_folder)
        # Save the conversation to a file named with the session ID
        file_path = os.path.join(thumbsup_folder, f"{session_id}.md")
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(conversation_history)
        return f"Session saved in Thumbsup folder at {file_path}."
    except Exception as e:
        return f"Error saving thumbs up: {str(e)}"
...
def handle_thumbs_down(session_id: str) -> str:
    ...
    try:
        ...
        conversation_history = "\n".join(f"{poster}: {message}" for poster, message in llsc.conversation_history)
        base_folder = os.path.dirname(LilLisaServerContext.SPEEDICT_FOLDERPATH)
        thumbsdown_folder = os.path.join(base_folder, "Thumbsdown")
        if not os.path.exists(thumbsdown_folder):
            os.makedirs(thumbsdown_folder)
        file_path = os.path.join(thumbsdown_folder, f"{session_id}.md")
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(conversation_history)
        return f"Session saved in Thumbsdown folder at {file_path}."
    except Exception as e:
        return f"Error saving thumbs down: {str(e)}"
...
with gr.Blocks(css=custom_css, title="Lil Lisa Server") as demo:
    ...
    with gr.Tab("Chatbot"):
        ...
        # New Feedback Buttons replacing the old endorsement radio/button
        with gr.Row():
    ...
    # Admin Tab with key verification and commands (no product or endorsed_by selection)
    with gr.Tab("Admin Panel"):
        ...
        def get_conversations(encrypted_key: str):
            try:
                jwt.decode(encrypted_key, AUTHENTICATION_KEY, algorithms="HS256")
                entry_names = os.listdir(LilLisaServerContext.SPEEDICT_FOLDERPATH)
                session_ids = [entry for entry in entry_names if os.path.isdir(os.path.join(LilLisaServerContext.SPEEDICT_FOLDERPATH, entry))]
                ...
                for session_id in session_ids:
                    try:
                        llsc = get_llsc(session_id)
                        if llsc.conversation_history and len(llsc.conversation_history) > 0:
                            useful_conversations.append((session_id, llsc.conversation_history))
                    except Exception as e:
                        continue
                if useful_conversations:
                    zip_stream = io.BytesIO()
                    with zipfile.ZipFile(zip_stream, "w") as zipf:
                        for session_id, conversation in useful_conversations:
        ...
        def handle_get_conversations(encrypted_key):
            content, content_type, filename = get_conversations(encrypted_key)
            if content_type == "application/zip":
                temp_file = tempfile.NamedTemporaryFile(delete=False, suffix=".zip")
                temp_file.write(content)
                temp_file.close()
