...
# --- Simple Welcome Message for Chat Initialization ---
...
with gr.Blocks(css=custom_css, title="Lil Lisa Server") as demo:
    ...
    about_me_popup = gr.HTML(
        ...
        <div id="about_me_popup" style="display:none;">
            <div style="background:white; color:black; padding:2rem; border-radius:var(--border-radius); box-shadow:var(--shadow-lg); max-width:600px; width:90%; text-align:left;">
                <h2>About LilLisa Chatbot</h2>
                <p>This chatbot is designed to help you with queries related to our products. Use the chat to ask your questions, and our system will provide relevant answers. For admin controls, switch to the Admin Panel tab.</p>
                <button onclick="document.getElementById('about_me_popup').style.display='none'">Close</button>
            </div>
        </div>
        """,
    ...
    about_me_btn = gr.HTML(
        value="""<button id="about_me_button" style="position: fixed; bottom: 20px; right: 20px; background-color: var(--primary); color: white; border-radius: var(--border-radius); padding: 0.75rem 1.5rem; border: none; font-weight: 600; cursor: pointer; box-shadow: var(--shadow-md);" onclick="document.getElementById('about_me_popup').style.display='flex'">About me</button>"""
    )
